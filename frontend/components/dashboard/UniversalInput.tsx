...
export function UniversalInput({ onSubmitted }: { onSubmitted?: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!trimmed || trimmed.length < 20 || trimmed.startsWith("http")) {
      ...
      timer = setTimeout(async () => {
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/ingest/preview`, {
            method: "POST",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/ingest`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const es = new EventSource(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/ingest/${data.job_id}/stream`)
      esRef.current = es
      ...
      es.onerror = () => {
        ...
        const poll = setInterval(async () => {
          attempts++
          try {
            const statusRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/ingest/${data.job_id}/status`)
