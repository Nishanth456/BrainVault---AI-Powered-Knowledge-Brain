...
export default function LearningPage() {
  ...
  const fetchSaved = () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/learning-path`)
      .then(r => r.json())
  ...
  const handleGenerate = async (topic: string) => {
    ...
    setCompletedStages([])
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/learning-path/generate`, {
        method: "POST",
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/learning-path/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleToggleStage = async (title: string) => {
    ...
    if (currentPath?.id) {
      try {
        await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/learning-path/${currentPath.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteSaved = async (id: string) => {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/learning-path/${id}`, { method: "DELETE" })
              ...
              <div className="space-y-3">
                {savedPaths.map(path => (
                  <SavedPathCard
                    ...
                    onOpen={() => {
                      fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/learning-path/${path.id}`)
                        .then(r => r.json())
                        .then(data => {
                          setCurrentPath(data)
