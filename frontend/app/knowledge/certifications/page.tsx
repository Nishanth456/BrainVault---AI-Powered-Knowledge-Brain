"use client"
import { CertCard, type CertItem } from "@/components/knowledge/CertCard"
import { Button } from "@/components/ui/button"
import { EmptyState } from "@/components/ui/EmptyState"
import { ChevronDown, ChevronRight, Award, RefreshCw } from "lucide-react"
import { useEffect, useMemo, useState } from "react"

function groupBySection(items: CertItem[]): Record<string, CertItem[]> {
  const groups: Record<string, CertItem[]> = {}
...
export default function CertificationsPage() {
  ...
  const fetchItems = () => {
    setLoading(true)
    setError(false)
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/knowledge/certifications`)
      .then(r => {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-zinc-500 text-sm">
              Your saved certifications, credentials, and associated exam topics.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {!loading && (
              <span className="text-sm text-zinc-600 hidden sm:block">
                {items.length} {items.length === 1 ? "cert" : "certs"}
              </span>
            )}
            <Button
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 text-zinc-500 gap-3">
            ...
            <p className="text-sm animate-pulse">Loading your certifications...</p>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="flex items-center justify-center py-20">
            <div className="bg-red-500/10 text-red-400 px-4 py-3 rounded-xl text-sm border border-red-500/20">
              Failed to load certifications. Is the backend running?
            </div>
          </div>
        )}
        ...
        {!loading && !error && items.length === 0 && (
          <div className="py-12">
            <EmptyState
              icon={<Award size={24} className="text-zinc-500" />}
              title="No certifications yet"
              description="Paste a credential or exam URL in the top bar to analyze and save it."
            />
          </div>
        )}
